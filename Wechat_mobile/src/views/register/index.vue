...
<div class="register-box">
    ...
    <van-form @submit="onSubmit" class="form-box" :show-error-message="false" @failed="onFailed">
        <van-field v-model="nickname" name="姓名" label="姓名" placeholder="请输入真实姓名" :rules="[{ required: true, message: '请输入真实姓名' }]" />
        <van-field v-model="mobile" name="手机号" type="number" maxlength="11" label="手机号" placeholder="请输入手机号" :rules="[{ required: true, message: '请填写手机号' }, { validator: validMobile, message: '手机号格式错误' }]" />
        <van-field v-model="area" name="所属区域" label="所属区域" placeholder="请输入所属区域" :rules="[{ required: true, message: '请填写所属区域' }]" />
        <van-field v-model="password" type="password" name="密码" label="密码" placeholder="请输入密码" :rules="[{ required: true, message: '请填写密码' }]" />
        <van-field v-model="againPassword" type="password" name="确认密码" label="确认密码" placeholder="请再次输入密码" :rules="[{ required: true, message: '请再次填写密码' }]" />
        <div class="btn">
