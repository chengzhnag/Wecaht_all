<template>
	<div class="add-box">
		<van-form @submit="onSubmit" :show-error-message="false" @failed="onFailed">
			<div class="content">
				<van-cell-group title="信息录入">
					...
					<van-field
						...
						size="large"
						required
						v-model="params.hydraulicName"
						name="水工姓名"
						label="水工姓名"
						placeholder="请输入水工姓名"
						:rules="[{ required: true, message: '请填写水工姓名' }]"
					/>
					...
					<van-field
						...
						size="large"
						required
						v-model="params.qualityAssuranceNum"
						name="质保卡号"
						label="质保卡号"
						placeholder="请输入质保卡号"
						:rules="[{ required: true, message: '请填写质保卡号' }]"
					/>
					...
					<van-field
						...
						size="large"
						required
						v-model="params.hydraulicIntegral"
						name="水工积分"
						label="水工积分"
						placeholder="请输入水工积分"
						:rules="[{ required: true, message: '请填写水工积分' }]"
					/>
					<div class="gap"></div>
			...
			<div class="btn-box"><van-button block type="info" native-type="submit">提交</van-button></div>
		</van-form>
	</div>
</template>
...
import { fileUpload, addCustomer } from '@/api';
...
export default {
	...
	methods: {
		...
		afterRead(file) {
			...
			this.funcUpload(formData, file.length);
		},
		funcUpload(formData, length) {
			fileUpload(formData)
				.then(res => {
					if (res.Code) {
						...
						this.fileList.splice(this.fileList.length - length);
