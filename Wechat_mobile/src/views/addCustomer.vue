        ...
        async afterRead(file) {
            ...
            if (file.length) {
                for (let i = 0; i < file.length; i++) {
                    let item = file[i];
                    let _size = item.file['size'] / 1024;
                    if (2 * 1024 < _size) {
                        let _data = await this.compressImg(item.content);
                        let _file = this.dataURLtoFile(_data, item.file.name)
                        formData.append('myfile', _file);
                    } else {
                        formData.append('myfile', item.file);
                    }
                }
            } else {
                let _size = file.file['size'] / 1024;
                if (2 * 1024 < _size) {
                    let _data = await this.compressImg(file.content);
					let _file = this.dataURLtoFile(_data, file.file.name);
                    formData.append('myfile', _file);
                } else {
